        ...
        for fullname in service_names {
            if let Some(info) = self.my_services.get(&fullname) {
                ...
                for intf in self.my_intfs.values() {
                    if let Some(_sock) = self.ipv4_sock.as_ref() {
                        self.unregister_service(info, intf.index, true);
                    }

                    if let Some(_sock) = self.ipv6_sock.as_ref() {
                        self.unregister_service(info, intf.index, false);
                    }
                }
            }
        }
